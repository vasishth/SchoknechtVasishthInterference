...
# Generate 5,000 simulations from model:
nsims<-5000
...
#### TWO CUE VERSION OF THE MODEL FROM SCHOKNECHT & VASISHTH  #####


# Load model 
source("interACT-2cues-sameclause.R")
reset_params()
...
## Create data frame with lf's, main effects and interaction simulated from model:
simdat_2cues <-data.frame(lfs=lfs,
                    MESyn=maineffectSyn,MESem=maineffectSem, InteractionSynSem=interactionSynSem)
## save to avoid recomputing each time:
save(simdat_2cues,file="simdat_2cues_sameclause.Rda")
...
round(c(mean = mean(simdat_2cues$MESyn), quantile(simdat_2cues$MESyn, probs = c(.025, .975))))
round(c(mean = mean(simdat_2cues$MESem), quantile(simdat_2cues$MESem, probs = c(.025, .975))))
round(c(mean = mean(simdat_2cues$InteractionSynSem), quantile(simdat_2cues$InteractionSynSem, probs = c(.025, .975))))

# combine sim data
simdat_3cues$model.version <- "three cues \n ({±grammatical subject}, \n {±same.clause} and {±animate})"
simdat_2cues$model.version <- "two cues \n ({±subject within same clause} \n and {±animate})"

generated_effects <- rbind(simdat_3cues, simdat_2cues)
...
generated_effects_long$model.version <- factor(generated_effects_long$model.version, levels=c("three cues \n ({±grammatical subject}, \n {±same.clause} and {±animate})", "two cues \n ({±subject within same clause} \n and {±animate})"))
...
ggsave("PriorPredicted_interACTversions.png", width=6, height=4, dpi=600)
