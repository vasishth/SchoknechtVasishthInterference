...
for (k in 1:10){
  ...
  # (we base this only on the semantic interference result and multiply by -1 for N400 effects)
  SEM<- abs(estimates[k,]$sem.train - (simdat_2cues_sameclause$MESem*-1)) <= (2*estimates[k,]$sem.train.SE)
...
for (k in 1:10){
  ...
  SEM<- abs(estimates[k,]$sem.train - (simdat_3cues$MESem*-1)) <= (2*estimates[k,]$sem.train.SE)
  table(SEM)
...
for (k in 1:10){
  # posterior latency factor (we base this only on the semantic interference result)
  SEM<- abs(estimates[k,]$sem.train - (simdat_1cue$MESem*-1)) <= (2*estimates[k,]$sem.train.SE)
