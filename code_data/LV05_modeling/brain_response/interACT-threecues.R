...
run <- function(d){
	...
	weights <- matrix(unlist(d$weights),nrow=total,ncol=3,byrow=TRUE)
...
weight_spreading_act <- function(Sji=matrix(c(0,0,0),1,3,T), weights=matrix(c(0.33,0.33,0.33),1,3,T), W=ga){
  Wkj <- matrix(unlist(c(0.33,0.33,0.33)),nrow=nrow(Sji),ncol=3,byrow=TRUE)
  rowSums(Wkj*Sji)
}
