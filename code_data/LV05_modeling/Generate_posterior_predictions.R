...
# load simulated data from all 3 models
...
SEM<- SVdat$lower[2]<=simdat_2cues_sameclause$MESem & simdat_2cues_sameclause$MESem<=SVdat$upper[2]
table(SEM)
...
SEM<- SVdat$lower[2]<=simdat_3cues$MESem & simdat_3cues$MESem<=SVdat$upper[2]
table(SEM)
...
# posterior latency factor (we base this only on the semantic interference result)
SEM<- SVdat$lower[2]<=simdat_1cue$MESem & simdat_1cue$MESem<=SVdat$upper[2]
