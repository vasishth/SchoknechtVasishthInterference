...
for i in range(1,151):
    ...
    epoch_files = glob.glob('epochs_distractor_item/*' + str(i).zfill(4) + '-epo.fif.gz')
    if (len(epoch_files) == 0):
        ...
        merged_files = 'merged_epochs_distractor/' + str(i).zfill(4) + '-epo.fif.gz'
        merged.save(merged_files, overwrite = True)
       
    
# subject 114 
epoch_files = glob.glob('epochs_distractor_item/*' + '114*' + '-epo.fif.gz')

epochs1 = mne.read_epochs(epoch_files[0],preload=True)
epochs2 = mne.read_epochs(epoch_files[1],preload=True)
epochs3 = mne.read_epochs(epoch_files[2],preload=True)
merged = mne.concatenate_epochs([epochs1, epochs2, epochs3])
merged_files = 'merged_epochs_distractor/' + '0114' + '-epo.fif.gz'
merged.save(merged_files, overwrite = True)
