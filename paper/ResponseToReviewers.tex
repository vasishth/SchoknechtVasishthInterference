...
Reviewer 2 primarily raised concerns regarding our experimental design. In response to these concerns, we have emphasized that we used a well-established design that was previously published in, among other journals, the \textcolor{red}{\textit{Journal of Memory and Language}}. We now stress in the paper that we are building on previously published work by directly comparing the statistical inferences, using Bayes factors, from the present and previous studies that use exactly the same design. 
...
We have used a well-studied design that was previously published in the \textcolor{red}{Journal of Memory and Language}, among other places. The previously published studies use exactly the same design, specifically the same syntactic retrieval cue, and found effects of syntactic interference \citep{vandyke07,mertzen}.
...
Additionally, we have added the following text to the General Discussion on page \textcolor{red}{X} to emphasize the theoretical grounding of our work:
...
As suggested, we have implemented a null model (no interference), leaving us with a total of three computational models (the null model, the three-cues model by \textcite{mertzen} and our newly proposed two-cues model). We use Bayes factors to quantitatively compare these models. The result of this model comparison is that the two-cues and three-cues models outperform the null model. Additionally, our new two-cues model performs slightly better than the three-cues model by \textcite{mertzen}. We have described the new modeling and model comparisons starting page \pageref{modeling}:
