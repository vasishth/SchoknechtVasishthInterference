...
Our work is situated in the cue-based retrieval framework which has been built over the last decades \citep{Lewis2005, nairne, mcelree}. Its main assumption is that sentence processing relies on skilled memory processes, with a focus on memory retrieval. We built on this assumption and focus on the question which specific retrieval cues are being used during subject-verb dependency processing. Our findings showed that the retrieval cues generated at the verb are highly specific (including hierarchical syntactic information) and can successfully identify the subject as the retrieval target. Therefore, the present findings further support the assumption that sentence processing relies on skilled memory retrieval. We have added the following text to the General Discussion on page \textcolor{red}{X} to emphasize this point:
