...
\textcolor{blue}{\section{Data availability statement}
...
The models included fixed effects for syntactic interference (high +0.5, low -0.5), semantic interference (high +0.5, low -0.5) and their interaction. \textcolor{blue}{In addition to the predictors of interest, we included trial id to account for potential effects of fatigue or adaptation to the task. The trial id of the 100 trials per session was recoded to span from 0 to 1. This was done to bring all fixed effects to a comparable scale which decreases the run time of the models. The models were run with full random effects (varying intercepts and slopes of all fixed effects and their interactions for participants and items).}  
...
Was there any evidence for the effects estimated above? The Bayes factor analysis is presented in Figure \ref{fig:spr_bfs}. The analyses use all the data from Experiments 1a and 1b. \textcolor{blue}{We found very to extremely strong evidence for the semantic interference effect in the pre-critical and spill-over region (BF$_{10}$ between 36 and 231). In the critical region, the evidence for semantic interference was inconclusive across the range of investigated priors (BF$_{10}$ between 0.7 and 3.5). There was strong evidence against an effect of syntactic interference under almost all priors in all regions (BF$_{10}$ < 0.5). Only in the spill-over region when small effects between -8 and 8 ms were assumed a priori, there was weak evidence for syntactic interference (BF$_{10}$ = 2.8). Similarly, there was overall evidence against the interaction of syntactic and semantic interference (BF$_{10}$ < 1). Only under the narrowest prior in the critical region, there was very weak evidence for the interaction (BF$_{10}$ = 1.4).}

\textcolor{blue}{All in all, the Bayes factor analysis provided very strong evidence for the semantic interference effect in the pre-critical and spill-over region, but inconclusive results in the critical region. In contrast, the Bayes factor analysis provided either at best weak evidence for syntactic interference and the interaction or even evidence against them.}
...
We presented the to-date largest-sample self-paced reading study that investigated the use of syntactic and semantic features during subject-verb dependency formation. \textcolor{blue}{Reading times at the critical verb were inconclusive for semantic interference and showed evidence against syntactic interference. We will discuss the absence of syntactic interference in our data in the general discussion. The semantic interference effect was already present at the pre-critical region; i.e., semantic interference occurred even before the retrieval event could begin.} The effects in the pre-critical region are discussed next.
...
\textcolor{blue}{Another possible explanation for the reading time differences starting at the distractor could be a difference in plausibility between the high and low semantic interference conditions. To explore this possibility, we conducted a web-based plausibility judgement experiment on PCIbex  \parencite{pcibex}. Since we were only interested in the potential plausibility difference between the semantic interference conditions, we used a one-factorial design for the plausibility rating experiment investigating semantic interference within the high syntactic interference conditions (see \ref{ex:hisyn}). Forty-four native speakers of German (mean age XX, age range: , w, m), who were recruited over prolific and had not participated in the SPR experiment or norming study, provided ratings on a scale from 1 (absolutely implausible) to 7 (absolutely plausible). The experiment included 40 implausible filler sentences which were created by exchanging nouns between the fillers used in the SPR experiment (e.g., Der Sohn des Kaisers, der den Feldzug gewonnen hatte, plante äußerst strategisch, weil er die Gießkanne an sich reißen wollte., `The son of the emperor who had won the campaign planned extremely strategically because he wanted to take the watering can by force.'). Participants spent approximately 37 minutes to complete the task and received 6.50 \pounds \, as compensation. The results of an ordinal brms model analyzing the ratings are shown in Figure X. Sentences with an inanimate distractor had a slightly higher probability to gain a high rating on the plausibility scale ($\beta$ = XX), i.e., the low semantic interference conditions are more plausible. To investigate whether this plausibility difference is driving the semantic interference effect in our SPR data, we ran an additional model which used the same specifications as described in the Statistical Analyses section with prior N(0,0.5) and additionally, the centered plausibility ratings as a fixed effect. Figure X presents the posteriors from this model. 
}
...
\begin{table}[!htbp]
  ...
  \cmidrule{2-3}
  \multirow{4}{1cm}{slope} & Normal(0, 0.1) & [-0.2, 0]\\
  & Normal(0, 0.5)& [-1, 1]\\
  & Normal(0, 1) & [-2, 2]\\
  & Normal(0, 2) & [-4, 4]\\
  \cmidrule{2-3}
...
Bayes factors are shown in Figure \ref{fig:eeg_bfs}. The Bayes factors provided very strong to \textcolor{blue}{moderate} evidence for the semantic interference effect for all priors 
\textcolor{blue}{(Normal$_{-}$(0, 0.1): BF$_{10}$ = 11,
Normal$_{-}$(0, 0.5): BF$_{10}$ = 80,
Normal$_{-}$(0, 1): BF$_{10}$ = 9,
Normal$_{-}$(0, 5): BF$_{10}$ = 5)}. 

By contrast, the syntactic interference effect had only anecdotal evidence  under the \textcolor{blue}{narrowest prior (Normal$_{-}$(0, 0.1): BF$_{10}$ = \textcolor{blue}{2.18}) and under the Normal$_{-}$(0, 0.5) prior there was no evidence for or against it (BF$_{10}$ = 1.05).} There was evidence \textit{against} syntactic interference under wider priors 
(Normal$_{-}$(0, 1): BF$_{10}$ = \textcolor{blue}{0.53},
Normal$_{-}$(0, 5): BF$_{10}$ = \textcolor{blue}{0.1}).  What this means is that, in our data, only if we assume a priori that the effect size is \textcolor{blue}{very small, i.e., maximally $-$0.2\,$\mu V$,} there is very weak evidence in favor of syntactic interference.
...
The Bayes factors analyses found \textcolor{blue}{very strong to moderate evidence for semantic interference in the N400 amplitude for the whole range of investigated effects sizes ($-$0.2 to $-11$\,$\mu V$)}. Syntactic interference had much weaker evidence in the N400 amplitude; only anecdotal evidence was found under \textcolor{blue}{the narrowest prior, which assumed effect sizes smaller than $-$0.2\,$\mu V$. No evidence for or against syntactic interference was found under a prior which assumed a reasonable effect size of up to $-1$\,$\mu V$ ($BF_{10}$=1.05). There was clear evidence against syntactic interference under wider priors (effect sizes of up to $-$2 or up to $-11$\,$\mu V$)}. The evidence against an interaction under all priors was unequivocal. \textcolor{blue}{In sum, the ERP results showed clear evidence for semantic interference and clear evidence against an interaction of syntactic and semantic interference. The evidence for or against syntactic interference was dependent on the a priori assumed effect size. However, even when very small effects were assumed a priori, the evidence for syntactic interference was weak at best. Consequently, we conclude that the ERP data showed no syntactic interference.} \textcolor{red}{Add conclusion after SPR results are revised.}
...
Could the absence of syntactic interference be reconciled with a cue-based retrieval account? We speculate that one possibility might be that the parser tracks hierarchical structure and that this hierarchical information is used when searching for a noun in memory. Specifically, the relevant syntactic cues that might be used at the verb are not  \{$\pm$ grammatical subject\} and \{$\pm$ same clause\} but rather a composite cue \{$\pm$ subject-in-same-clause\}. This proposal is in the spirit of the  \textcite{dillon2013} and \textcite{Sturt2003} claim, that the parser intelligently targets only the syntactically relevant noun during antecedent retrieval.  In the present case, it is possible that syntactically relevant noun is only the one in the same clause as the verb.
...
\textcolor{red}{Rewrite this section to reflect the new discussion.} In the remainder of this section, we discuss two important issues that relate to previous experiments that investigated syntactic and semantic interference. The first is the role of encoding vs.\ retrieval interference, and the second is broader implications of our findings in the context of previous related work.

\subsection{Comparison with previous findings}
\textcolor{blue}{The present study used the same design as two previous studies \parencite{vandyke07, mertzen}. Figure \ref{fig:previous_vs_present} A shows the estimated reading times differences from the previous studies and our self-paced reading experiment. Because the \textcite{vandyke07} data was not available to us, we derived estimates for their study from the summary statistics in the published paper. \textcite{mertzen} made their data available. In comparison to the previous estimates, the effects of the present study are relatively small and they are associated with less uncertainty. To see whether there is evidence for the estimated effects, we computed Bayes factors for the previous studies. For \textcite{vandyke07}, we used the Normal-Normal conjugate case to derive posteriors from the summary statistics (mean and SE). Then using these posteriors and a reasonable prior (Normal(0,0.5), a priori assumed effect size in the range of -40 ms to 40 ms), we computed Bayes Factors using the Savage-Dickey method. Bayes factors for \citeauthor{mertzen}'s (\citeyear{mertzen}) data were computed using the Savage-Dickey method from models which we run on their data along to our own analyses described in Statistical Analyses for Experiments 1a and b. The Bayes factors of the effects of interest in self-paced reading times and total fixation times of the present and previous studies under prior Normal(0,0.5) are shown in Figure \ref{fig:previous_vs_present} B.}
...
\textcolor{blue}{The Bayes factors for the semantic interference effect consistently provide evidence for the effect in the pre-critical region. In the critical region, most experiments provide evidence against semantic interference, the two self-paced reading experiments, being the exceptions, provide evidence for the effect. In the spill-over region, the Bayes factor results for semantic interference are mixed: the two self-paced reading experiments provide extremely strong evidence for the effect, the total fixation times for two of the eye-tracking experiments provide weak evidence for the effect and the total fixation times of two other eye-tracking experiments provide evidence against the effect. Importantly, the clustering in the total fixation time results does not align with the authors of the studies.}

\textcolor{blue}{The Bayes factors for the interaction in all regions are rather consistent between studies and are either inconclusive or provide evidence against the interaction. The only exception is the very large Bayes factor of \citeauthor{vandyke07}'s (\citeyear{vandyke07}) Experiment 1. In sum, while the present and previous studies show comparable region-wise results for semantic interference and the interaction, they differ remarkably regarding syntactic interference in the pre-critical and critical region. In the pre-critical region, the present study is the obvious outlier, being the only study to provide evidence against the effect. In the critical region, four of the investigated experiments show weak evidence for or against syntactic interference with two outliers: \citeauthor{vandyke07}'s (\citeyear{vandyke07}) Experiment 2 providing extreme evidence for the effect and the present study providing strong evidence against it. In conclusion, the most remarkable difference between the present study and previous findings concerns the syntactic interference in the pre-critical region.}

%The difference in observed effects \textcolor{blue}{between our SPR experiment and the other reading time studies} could be explained by differences in statistical power or by methodological differences.



The present study has higher statistical power than the previous studies. \citet{vandyke07} had 35 - 40 participants and 36 - 48 items in each of her three experiments, respectively. \textcite{mertzen} tested 61 English speakers and 121 German speakers with 40 items each. In contrast, we tested 774 participants with at least 60 items each (a subset of 160 participants read all 120 items). Therefore, the present study is the study with the highest power to date using this 2 $\times$ 2 interference design. Consequently, the estimates from the present study are the most precise ones so far for this design.\textcolor{blue}{This is reflected by the tighter estimates in Figure \ref{fig:previous_vs_present}}. The smaller effect size in the present study is a consequence of higher power, and the magnitude of the effects observed is comparable to that of other large-scale reading studies \citep{nicenboim} and meta-analyses on interference \citep{jaeger_etal_2017}.

An obvious difference from previous studies is the experimental methodology. The majority of the previous experiments employed eye-tracking while reading, while we used a self-paced reading task. Self-paced reading (SPR) might have a greater tendency to be affected by interference compared to natural reading (as in eye-tracking studies), due to the restricted reading format in SPR: Comprehenders have no opportunity to revisit previous material during conventional self-paced reading \citep[but see][]{BSPR}. SPR is likely to be more demanding on the comprehender's memory, which might make it easier to detect interference effects. This argumentation is supported by our estimation of the results of \citeauthor{vandyke07}'s (\citeyear{vandyke07}) Experiment 1, the only other SPR experiment using this design: 
...
So, why did we find a semantic interference effect in all regions, whereas \citet{mertzen} only found one in the pre-critical region? Apart from \textcolor{blue}{the additional distractor and} the methodological difference (eye-tracking vs.\ self-paced reading), a plausible  explanation for the divergence is lower power in \citeauthor{mertzen}'s (\citeyear{mertzen}) study. While they already had considerably more participants and items than many psycholinguistic studies, in the present study the number of participants was much higher than in theirs (774 vs.\ 121) and the number of items was also higher (60/120 vs.\ 40). 
