...
%It is informative to fit the model\textcolor{blue}{s} to the data and then examine the posterior predictive distribution of the \textcolor{blue}{ERPs} for each effect. We generated posterior predictive distributions for the effects of interest by fitting the $F$ parameter to the \textcolor{blue}{ERP} data. To avoid overfitting, we fitted $F$ only to the semantic interference effect (and not the syntactic interference effect or interaction). This was done by implementing an Approximate Bayesian Computation approach \parencite{SissonABC}: the model's predictions are generated iteratively, and only those parameter values for the scaling parameter $F$ are retained that predict values for the semantic interference effect that are close to the observed effect size $d$ in the \textcolor{blue}{ERP} data. `Close' is defined as $d \pm \,2\times SE$ ms; for details of this approach as applied to the cue-based retrieval model, see  \textcite{vasishth2020_abc}. 

%\textcolor{blue}{Out of 200,000 iterations of ABC algorithm, we obtained only 40,748 samples of $F$ for the 3-cues model, 28,718 for the 2-cues model and 6,615 for the 1-cue model. This is not surprising given the relatively wide prior on $F$. In future work, the posterior distribution, with range [-0.5, -0.1]\,$\mu V$, could be used as a basis for redefining a tighter prior on the scaling parameter $F$. Figure \ref{fig:model_predictions_ERP} shows the  posterior predictive distributions of the effects of interest in comparison to the prior predictive distribution and our ERP data. The prior and posterior predictions from the model with three cues show equally large effects of syntactic and semantic interference and an interaction. In contrast, the model using two cues, including a syntactic cue that utilizes syntactic hierarchy, predicts no effect of syntactic interference and no interaction, but a semantic interference effect. The model using one cue predicts no interference effects. The prior and posterior predictions of all models overlap with the effects of interest in the data. }

\Copy{figpriorposteriordata}{
\begin{sidewaysfigure}[htpb]
    \caption{The prior predictions on the microvolt scale, i.e., for negative ERPs, for syntactic and semantic interference and their interaction \textcolor{blue}{from three versions of the Lewis and Vasishth (2005) cue-based retrieval model.}
...
\textcolor{blue}{We evaluate whether our new two-cues model quantitatively outperforms the three-cues model and the one-cue model in its predictive performance on the semantic interference data. To evaluate the relative performance of the models, we use the Bayes factor analysis. We compute Bayes factors for the two-cues model compared with the three-cues model and the one cue model. The Bayes factor obtained for the two-cues model would imply the evidence in favor of this model when it is compared against the other two models.   
Since Bayes factors are sensitive to the choice of priors on the parameters of interest. We compute Bayes factors under six different priors for the latency factor parameter $F$,
$Normal$(0.01,0.01), 
$Normal$(0.01,0.02),
$Normal$(0.01,0.005),
$Normal$(0.02,0.01),
...
The procedure for computing Bayes factors is described as follows. The Bayes factor computation requires us to estimate the marginal likelihood of the competing models. We used the Monte Carlo integration method to estimate the marginal likelihood of each model. First, we chose an importance density for the parameter $F$ from which we draw $0.2$ million samples of $F$. Then, for each of these proposed parameter values, we compute the likelihood and prior density. The estimated marginal likelihood of a model will be given by 

\begin{equation}
ML = \frac{1}{n} \sum_{i=1}{n} \mathcal{L}{F_i|y} p(F_i)  \text{ where } F_i \sim Normal_{lb=0,ub=0.05}(0.0.025) 
\end{equation}

\noindent where $y$ is the observed data, i.e., observed semantic interference effect in our case, $F_i$ is the $i^{th}$ proposal from the importance density $Normal_{lb=0,ub=0.05}(0.0.025) $. The term $\mathcal{L}{F_i|y}$ gives the likelihood of proposal $F_i$ given the data $y$ and $p(F_i)$ gives the prior density of $F_i$. We compute marginal likelihoods under different prior assumptions $p(F)$.

The LV05 model is a complex, non-deterministic model and it is difficult the express the model's likelihood anallytically without drastically simplifying the model cite[e.g., in ][]{NicenboimRetrieval2018,lisson2020computational}. In absence of an analytically-expressed likelihood function, we use Approximate Bayesian Computation (ABC)~\citep{sisson2018handbook,palestro2018likelihood} approach to estimate approximate likelihood for a given value of $F$. Under this approach, we compare the model generated data conditioned on $F=F_i$ and the actual observed data $y$. If the model-generated data $x_{i} \sim Model(F_i)$ for $F_i$ is close to the actual data $y$, then $F_i$ has higher likelihood and vice versa. The likelihood term can be rewritten as $\mathcal{L}{F_i|y} = \Psi(d(x_i, y), 0 , \delta)$, where $\Psi(.|\delta)$ represent a density kernel that weighs a proposal $F_i$ based on ditance between model-generated data and actual data, $d(x_i,y)$. 

Using the above method, we estimate marginal likelihood for each candidate model under six different prior assumptions. The Bayes factor in favor of a model M1
compared to a model M2, i.e., BF12 is computed as the ratio
of the marginal likelihoods of M1 and M2. The Bayes factor BF12 represents the extent to which the model M1 is more
...
From the marginal likelihoods, we compute Bayes factors for the two-cues model compared against the three-cues model and the one-cue model. That is, the marginal likelihood of the 2-cues model $\mathrm{M1}$ was divided by the marginal likelihood of the three-cues model $\mathrm{M2}$ and one-cue model $\mathrm{M3}$ to obtain the Bayes factors $BF_{12}$ and $BF_{13}$ respectively. Following the convention from \citep{jeffreys1998theory}, if the Bayes factor value is larger than $10$. it would be interpreted as strong evidence in favor of the two-cues model and if the Bayes factor is between $3$ and $10$, it is interpreted as moderate evidence in favor of the two-cues model compared against the competing model. 
