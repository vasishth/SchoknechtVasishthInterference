...
\textcolor{blue}{The \citet{Lewis2005} model is generally used to predict reading times. Recently, \textcite{mertzen} presented simulations for the present design that predicted approximately equally sized interference effects 
for both the syntactic and semantic manipulations at the critical verb. Since we have found that the reading times at the critical verb, i.e., the retrieval site, were confounded by pre-critical effects, which were best explained by \textit{encoding} interference, it makes no sense to compare the results from the present SPR experiment to predictions from the Lewis and Vasishth (2005) model of sentence processing
as skilled memory retrieval. Instead we use the Lewis and Vasishth (2005) model to generate predictions on the microvolt scale for ERP data by adjusting the scaling factor $F$ which usually links the activation of items in memory to retrieval times \citep{Lewis2005,anderson_etal_2004_ACT_R} to link memory activation to brain responses.} 


The modeling reported here is, to the best of our knowledge, the first time that the \citet{Lewis2005} model has been fit quantitatively to ERP data. We sampled the free scaling parameter $F$ from a uniform prior distribution ($Uniform(0.005, 0.04)$). This prior reflects the a priori belief that effects will range from 0.5 to 4 $\mu V$ with equal likelihood for all effect sizes in this range. This uninformative prior was chosen because there exists no previously published ERP data on the Van Dyke (2007) design. After sampling the scaling parameter, simulated data was generated from the model while holding all other parameters in the model at fixed values. \textcolor{blue}{We decided to model N400 amplitude differences because Bayes factors provided stronger evidence for the semantic interference effect on the N400 than on the P600 in the present ERP study. To reflect that larger predicted effect sizes correspond to more negative N400 amplitudes, the predicted effects from the model} were multiplied with $-1$. To avoid overfitting we fitted $F$ only to the semantic interference effect in order to generate posterior predictive distributions of the syntactic and semantic interference effects. 

\textcolor{blue}{
For their recent simulations, \textcite{mertzen} used a version of the Lewis and Vasishth (2005) model that assumes that in the \textcite{vandyke07} design, three cues are used at the verb: \{$\pm$ grammatical subject\}, \{$\pm$ same clause\} and \{$\pm$ animacy\}. Although it is clear that our data does not support equally sized effects of syntactic and semantic interference, we decided to use their version of the model for our modeling because it has been established as a good model for the present design. We have implemented two alternative versions of the model. A model which puts more emphasis on hierarchical syntactic structure. Given the evidence against syntactic interference in our data, it is plausible that, at least in this experimental design and language, syntactic interference plays little to no role. We speculate that this could be due to the parser tracking} hierarchical structure and that this hierarchical information is used when searching for a noun in memory. Specifically, the relevant syntactic cues that might be used at the verb are not \{$\pm$ grammatical subject\} and \{$\pm$ same clause\} \textcolor{blue}{-- as implemented by \textcite{mertzen}} -- but rather a composite cue \{$\pm$ subject-in-same-clause\}. \textcolor{blue}{This idea actually goes back to the roots of the Lewis and Vasishth (2005) model by making the model ``sensitive to whether or not it is parsing an embedded clause'' \citep[][p.388]{Lewis2005}. The resulting model assumes two retrieval cues: \{$\pm$ subject-in-same-clause\} and \{$\pm$ animate\}.} This proposal is \textcolor{blue}{furthermore} in the spirit of the \textcite{dillon2013} and \textcite{Sturt2003} claim, that the parser intelligently targets only the syntactically relevant noun during antecedent retrieval. In the present case, it is possible that syntactically relevant noun is only the one in the same clause as the verb. \textcolor{blue}{Lastly, we implemented a model with one cue which unambiguously matches the subject. This cue would be the master composite cue \{$\pm$ animate-subject-in-same-clause\}. This model naturally does not predict any interference effects. }

It is informative to fit the models to the data and then examine the posterior predictive distribution of the \textcolor{blue}{ERPs} for each effect. 
We generated posterior predictive distributions for the effects of interest by fitting the $F$ parameter to the \textcolor{blue}{ERP} data. This was done by implementing an Approximate Bayesian Computation approach \parencite{SissonABC}: the model's predictions are generated iteratively, and only those parameter values for the  scaling parameter $F$ are retained that predict values for the semantic interference effect that are close to the observed effect size $d$ in the \textcolor{blue}{ERP} data. `Close' is defined as $d \pm \,2\times SE$ ms; for details of this approach as applied to the cue-based retrieval model, see  \textcite{vasishth2020_abc}. The prior and posterior predictions for the effects of interest on the microvolt scale in comparison to our ERP data are shown in Figure \ref{fig:model_predictions_ERP}.
...
\textcolor{red}{To do (Pia): Describe figure\\
model comparison (cross-validation)}
%For the ERP modeling, 473 out of 20,000 sampled values of $F$ lead to simulated effects within the range of the observed effect and were thus used for posterior predictions. The prior and posterior predictions of the model overlap with the effects of interest in the data. The general effect pattern (no syntactic interference effect \textcolor{blue}{and no interaction}, but a semantic interference effect) is similar in both the prior and posterior predictive distributions. Therefore, the assumption that the relevant syntactic cue is the \{$\pm$ subject-in-same-clause\} cue -- rather than the previously assumed \{$\pm$ grammatical subject\} cue --  seems reasonable for the present ERP study.
