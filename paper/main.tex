...
\textcolor{blue}{Out of 200,000 iterations of ABC algorithm, we obtained only 40,748 samples of $F$ for the 3-cues model, 28,718 for the 2-cues model and 6,615 for the 1-cue model. This is not surprising given
...
\textcolor{blue}{To evaluate whether our new 2-cues model quantitatively outperforms the two other models in its predictive fit, we computed Bayes factors for the relevant model pairs under six different priors for the $F$ parameter 
($Normal$(0.01,0.01), 
$Normal$(0.01,0.02),
$Normal$(0.01,0.005),
$Normal$(0.02,0.01),
$Normal$(0.02,0.02),
$Normal$(0.02,0.005)). First, we computed the likelihood of the sampled $F$ parameters under each of the priors (LKL$_{F-prior}$) and the sampling prior (LKL$_{F-model}$). Then, the likelihood of the predicted semantic interference effect from the model was computed given the size of the observed semantic interference effect (LKL$_{predicted-effect}$). To obtain the marginal likelihood of the model, the product of LKL$_{predicted-effect}$ and LKL$_{F-prior}$ was divided by LKL$_{F-model}$. The marginal likelihood of the 2-cues model was then divided by the marginal likelihood of the three-cues and one-cue model, respectively. The resulting Bayes factors are shown in Figure \ref{fig:BF_compmods}. Compared to the null model with one cue, the Bayes factors provide extreme evidence for the new two-cues model. Therefore, we can conclude that the computational modeling shows that our data shows interference. Compared to the three-cues model proposed by \textcite{mertzen}, the Bayes factors provide moderate evidence for the new two-cues model. Therefore, the assumption that the relevant syntactic cue is the \{$\pm$subject-in-same-clause\} cue – rather than the previously assumed \{$\pm$grammatical subject\} cue – is moderately supported by these modeling results.
}
