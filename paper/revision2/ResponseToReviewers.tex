...
We do not share the reviewer's interpretation that the effects in the later regions are meaningless. We use the pre-critical reading times as a proxy to investigate the effect of encoding interference caused by the distractors. We emphasize the link between the distractor and the pre-critical region by discussing them together in a new section with the title ``Reading times difference prior to retrieval" (starting on page \pageref{prior_to_retrieval}). This new section replaces the former section ``Pre-critical effects". 

\begin{quote}
    \Paste{SPR_discussion_revised}
\end{quote}

\textcolor{red}{Figure out how much of the revised text to paste here}
...
We have revised abstract to emphasize that our findings only apply to the present design. We paste the revised abstract here:
